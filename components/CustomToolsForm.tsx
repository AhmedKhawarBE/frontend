...
const extractPlaceholdersFromString = (str: string): string[] => {
  ...
  const handleSubmit = async () => {
    try {
      // Transform arrays into objects for API
      const formatKV = (arr: KVEntry[]) =>
        Object.fromEntries(arr.filter(e => e.key).map(e => [e.key, e.value]))
      ...
      toast({
        title: "✨ Success",
        description: "Your custom tool was created successfully!",
      })
      ...
      setFormData({
        ...
        omit_nulls: false,
        timeout_ms: 30000,
        speak_during_execution: { enabled: false, delay_seconds: 0, message: "" },
      })
    } catch (err: any) {
      toast({
        title: "🚨 Error",
